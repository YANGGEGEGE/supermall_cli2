  ...
  import CheckButton from './CheckButton'
    ...
    computed: {
        ...
        return cartList.filter(item => {
          ...
          //这里是 0就是preValue的初始化值
          return preValue + item.count * item.price
        }, 0).toFixed(2) 
      },
      checkLength(){
        const cartList2 = this.$store.getters.cartList;
        return cartList2.filter(item => item.checked).length
      },
      isSelectAll: function () {
          if(this.$store.getters.cartList ===0) return false
